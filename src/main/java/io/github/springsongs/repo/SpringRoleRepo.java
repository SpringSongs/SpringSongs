...
public interface SpringRoleRepo extends JpaRepository<SpringRole, String> {
	...
	@Query(value = "select new io.github.springsongs.domain.dto.RoleCodeDTO(bre.id,bre.title) from SpringRole bre left join SpringUserRole bure on bre.id=bure.roleId left join SpringUser bue on bue.id=bure.userId where bure.userId=:userId")
